...
if (controles_timer > 0) {
	...
	var _bx = x + 132;
	var _by = y - 70 + sin(current_time * 0.006) * 3;
	draw_set_alpha(0.78 * _alpha);
	draw_set_color(make_color_rgb(5, 13, 26));
	draw_roundrect(_bx, _by, _bx + 220, _by + 104, false);
	draw_set_alpha(_alpha);
	draw_set_color(make_color_rgb(122, 72, 255));
	draw_roundrect(_bx, _by, _bx + 220, _by + 104, true);
	draw_set_font(fnt_dialogo);
	draw_set_halign(fa_center);
	draw_set_valign(fa_middle);
	draw_set_color(make_color_rgb(186, 118, 255));
	draw_text_transformed(_bx + 110, _by + 34, "WASD / SETAS", 0.82 * _pulse, 0.82 * _pulse, 0);
	draw_set_color(make_color_rgb(255, 218, 116));
	draw_text_transformed(_bx + 110, _by + 72, "K  ATIRAR", 0.82 * _pulse, 0.82 * _pulse, 0);

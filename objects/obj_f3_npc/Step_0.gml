...
if (atualizar_felicidade > 0){
	...
	if (idbackground){
		switch (estado_felicidade){
			case 0:
				layer_background_sprite(idbackground, spr_f3_fundo_feliz);
				break;
				
			case 1:
				layer_background_sprite(idbackground, spr_f3_fundo_serio);
				break;
				
			case 2:
				layer_background_sprite(idbackground, spr_f3_fundo_triste);
				break;
		}
	}
	...
	with (obj_f3_controlador_msg){
		instance_destroy();
	}
	
	alarm[0] = 120;
	audio_stop_sound(snd_f3_musica);
	ns_audio_play_sfx(snd_f3_fim_jogo, 1, 0, 1, 0, 1);

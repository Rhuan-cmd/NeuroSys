...
if (cutscene_clickou || transicao_caixa || ativo || estado_final != 0) {
    ...
    draw_sprite_ext(spr_f2_close, 0, botao_fixo_x, botao_fixo_y, 0.78, 0.78, 0, c_white, 1);
}
...
draw_sprite_ext(spr_f2_status_lives, vidas, 831, 99, s_lives, s_lives, 0, c_white, intro);

var frame_tempo = 10;
if (ativo) {
    frame_tempo = max(0, min(10, ceil((limite_clique - timer_clique) / room_speed)));
}
draw_sprite_ext(spr_f2_status_timer, frame_tempo, 831, 274, s_timer, s_timer, 0, c_white, clamp((status_intro_timer - 8) / 38, 0, 1));
...
draw_sprite_ext(spr_f2_status_score, min(cliques, cliques_necessarios), 831, 445, s_score, s_score, 0, c_white, clamp((status_intro_timer - 16) / 38, 0, 1));
